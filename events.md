...
|ticket_options		|[Ticket Options[]](#ticket-options)	|An array of object hashes representing the different ticket options available for purches. (ex: $5 general admission tickets and $50 VIP tickets)
...
|Name          |Type      |Description
|-----------    |-----------|--------------
|ticket_option.title	|string    |The name of the ticket option. (ex: "General admission")
|ticket_option.description	|string    |A longer description of the ticket option. (ex: "Gets you access to the convention floor.")
|ticket_option.amount	|float    |The price of each ticket for this ticket option, in the specified currency. (ex: "10.50")
|ticket_option.currency	|string    |ISO 4217 designation of currency. (ex: "USD", "JPY")
|ticket_option.limit	|integer    |The total amount of tickets available for this ticket option, not including tickets already sold. (ex: There may be 10 tickets of this ticket option available and five already sold, but the ```limit``` field would still read "10".)
|ticket_option.total_tickets		|integer	|A read-only computed property representing the current count of the total number of tickets sold for the ticket option.
|ticket_option.total_amount		|float	|A read-only computed property representing the current count of the total amount of money generated by selling tickets for the ticket option.
